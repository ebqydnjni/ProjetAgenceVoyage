...
if (isset($_POST['sDidVol'])) {
    ...
    $stmt = $bdd->prepare("UPDATE VOLS SET softDelete = 2  WHERE idVol = :id");
...
if (isset($_POST['newIdVol'], $_POST['newNumVol'],$_POST['newDateVol'],$_POST['newJourSemaine'],$_POST['newHeureDepart'],$_POST['newHeureArrive'],$_POST['distance'],$_POST['depart'],$_POST['arrive'])) {
    ...
    $depart = $stmt->fetch(PDO::FETCH_ASSOC)['IATA_CODE'];
    $stmt = $bdd->prepare("SELECT IATA_CODE FROM aeroports where nomAeroport = :arrive");
    $stmt->execute(array(':arrive'=>$arrive));
    $arrive = $stmt->fetch(PDO::FETCH_ASSOC)['IATA_CODE'];
    $stmt = $bdd->prepare("INSERT INTO vols (numVol, dateVol, jourSemaine, heureDepart, heureArrive, distance, Adepart, Aarrive) VALUES (:numVol, :dateVol, :jourSemaine, :heureDepart, :heureArrive, :distance, :depart, :arrive)");
    $stmt->execute(array(':numVol' => $newNumVol, ':dateVol' => $newDateVol, ':jourSemaine' => $newJourSemaine, ':heureDepart' => $newHeureDepart, ':heureArrive' => $newHeureArrive, ':distance' => $distance, ':depart' => $depart, ':arrive' => $arrive));
}
...
<form action="adminBillets.php" method="post"> <input type="hidden" name="sDidVol" value="<?php echo $billet['idVol']; ?>">
...
echo '<h2>Modifier</h2>';
echo '<form  action="adminBillets.php" method="post" id="signUp">';
 ...
 <?php endforeach; ?>
  ...
  <div class="popup-content">
    <div class="sign_up_form">
      ...
      <h2>Ajouter</h2>
      <form action="adminBillets.php" method="post" id="signUp">
...
<input type="text" placeholder="Départ" list="liste_departs" name="depart" id="" />
...
<input type="text" placeholder="Arrivée" list="liste_arrivees" name="arrive" id="" />
